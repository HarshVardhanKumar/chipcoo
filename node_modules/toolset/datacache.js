	...
	if (this.options.TTL) {
		setInterval(function() {
			...
			for (i in scope.files) {
				if (scope.files[i].expires <= new Date().getTime()) {
					delete scope.files[i];
				}
			}
		}, this.options.TTL);
	}
...
datacache.prototype.get = function(type, options, callback) {
	var scope = this;
	var id = this.options.type[type].toString(options);
	if (this.cache[id]) {
		callback(this.cache[id].data);
	} else {
		// Get the data
		this.options.type[type].method(options, function(response) {
			scope.cache[id] = {
				expires: 	new Date().getTime()+scope.options.TTL,
				data:		response
			};
			callback(scope.cache[id].data);
		});
	}
}
